...
int remove_friend(string user1, string user2){
    ...
    if(!exist){
        ...
        sql = "DELETE FROM FRIENDS WHERE USER1 = '" + user1 + "' AND USER2 = '" + user2 + "' AND TYPE = 'friend';";
    ...
    if(!exist){
        ...
        sql = "DELETE FROM FRIENDS WHERE USER1 = '" + user2 + "' AND USER2 = '" + user1 + "' AND TYPE = 'friend';";
        rc = sqlite3_exec(db, sql.c_str(), NULL, 0, NULL);
        if(rc != SQLITE_OK) cerr << "error remove friend." << endl;
        else cout << "Friend deleted." << endl;
        return 1;
...
int add_friend(string user1, string user2){
    ...
    sql = "INSERT INTO RELATION (USER1, USER2, CHAT_ID) VALUES('"
                        + user1 + "', '" + user2 + "', null);";
    rc = sqlite3_exec(db, sql.c_str(), NULL, 0, NULL);
    if(rc != SQLITE_OK){
...
int unblock(string user1, string user2){
    ...
    //check if there exist any relation
    sql = "DELETE FROM RELATION WHERE USER1 = '" + user1 + "' AND USER2 = '" + user2 + "' AND TYPE = 'block';";
    rc = sqlite3_exec(db, sql.c_str(), NULL, 0, NULL);
    if(rc != SQLITE_OK) cerr << "error remove friend." << endl;
}